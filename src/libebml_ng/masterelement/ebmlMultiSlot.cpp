...
#include "libebml_ng/masterelement/ebmlMultiSlot.h"
#include "libebml_ng/masterelement/ebmlMultiSlot.tpp"
...
namespace ebml {
    ...
    : name(name), _childClasses(childSpec), _multi(multielem) {
        // this->name = name;
    ...
    // childClassSpec_t _consolidateSlotSpec(const slotSpec_l& slotSpecs) {
    ...
    : ebmlMultiSlotClass(unpackVint(ebmlID), name, slotSpecs, &seekHelper) {}

    ebmlMultiSlotClass::ebmlMultiSlotClass(const char* ebmlID, const std::wstring& name, const slotSpec_l& slotSpecs, const seekHelper_t* seekHelper)
    : ebmlMultiSlotClass(unpackVint(ebmlID), name, slotSpecs, seekHelper) {}
    ...
    // ebmlMultiSlotClass::ebmlMultiSlotClass(
    //     const char* ebmlID, const std::wstring& name, const slotSpec_l& slotSpecs, const occurSpec_t& recursive, size_t recurseslot, const seekHelper_t* seekHelper)
    // : ebmlMultiSlotClass(unpackVint(ebmlID), name, slotSpecs, recursive, recurseslot, seekHelper) {}

    ebmlMultiSlotClass::ebmlMultiSlotClass(ebmlID_t ebmlID, const std::wstring& name, const slotSpec_l& slotSpecs)
    : ebmlMultiSlotClass(ebmlID, name, slotSpecs, &seekHelper) {}

    ebmlMultiSlotClass::ebmlMultiSlotClass(ebmlID_t ebmlID, const std::wstring& name, const slotSpec_l& slotSpecs, const seekHelper_t* seekHelper)
    : ClsMixin<ebmlMultiSlotClass, ebmlMultiSlot, ebmlMasterElementClass>(ebmlID, name, seekHelper) {
        for (const auto& slotSpec : slotSpecs) {
            this->_slotSpecs.push_back(slotSpec);
        }
        ...
        for (const auto& slotSpec : this->_slotSpecs) {
            ...
            for (const auto& childClassSpec : childClasses) {
                const auto& occurSpec = childClasses.occurSpec(childClassSpec.first);
                this->_childClasses.add({childClassSpec.second, occurSpec.min, occurSpec.max});
                this->_slots_by_ebmlID[childClassSpec.first] = &slotSpec;
            }
        }
    }
    ...
    // : ebmlMultiSlotClass(ebmlID, name, slotSpecs, recursive, recurseslot, &seekHelper) {}
    //
    // ebmlMultiSlotClass::ebmlMultiSlotClass(
    //     ebmlID_t ebmlID, const std::wstring& name, const slotSpec_l& slotSpecs, const occurSpec_t& recursive, size_t recurseslot, const seekHelper_t* seekHelper)
    // : ebmlMultiSlotClass(ebmlID, name, slotSpecs, seekHelper) {
    //     if (recursive.max != 0) {
    ...
    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls, const slotArg_l& args)
    : InstMixin<ebmlMultiSlotClass, ebmlMultiSlot, ebmlMasterElement>(cls) {
        _validateArgs(args);
        _validateData(args);
        _setData(args);
    }

    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls, slotArg_l&& args)
    : ebmlMultiSlot(cls) {
        _validateArgs(args);
        _validateData(args);
    ...
    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls, const slotArg_d& kwargs)
    : ebmlMultiSlot(cls) {
    ...
    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls, slotArg_d&& kwargs)
    : ebmlMultiSlot(cls) {
        _validateArgs(kwargs);
        _validateData(kwargs);
    ...
    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls, const slotArg_l& args, const slotArg_d& kwargs)
    : ebmlMultiSlot(cls) {
    ...
    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls, const slotArg_l& args, slotArg_d&& kwargs)
    : ebmlMultiSlot(cls) {
        _validateArgs(args, kwargs);
        _validateData(args);
        _validateData(kwargs);
    ...
    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls, slotArg_l&& args, const slotArg_d& kwargs)
    : ebmlMultiSlot(cls) {
        _validateArgs(args, kwargs);
        _validateData(args);
        _validateData(kwargs);
    ...
    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls, slotArg_l&& args, slotArg_d&& kwargs)
    : ebmlMultiSlot(cls) {
        _validateArgs(args, kwargs);
        _validateData(args);
        _validateData(kwargs);
    ...
    // ebmlMultiSlot_sp ebmlMultiSlotClass::operator()(const slotArg_l& args) const {
    //     auto elem = new ebmlMultiSlot(this);
    //     auto elem_sp = ebmlMultiSlot_sp(elem);
    ...
    // }
    //
    // ebmlMultiSlot_sp ebmlMultiSlotClass::operator()(slotArg_l&& args) const {
    //     auto elem = new ebmlMultiSlot(this);
    //     auto elem_sp = ebmlMultiSlot_sp(elem);
    //     elem->_validateArgs(args);
    //     elem->_validateData(args);
    ...
    // }
    //
    // ebmlMultiSlot_sp ebmlMultiSlotClass::operator()(const slotArg_d& kwargs) const {
    //     auto elem = new ebmlMultiSlot(this);
    //     auto elem_sp = ebmlMultiSlot_sp(elem);
    ...
    // }
    //
    // ebmlMultiSlot_sp ebmlMultiSlotClass::operator()(slotArg_d&& kwargs) const {
    //     auto elem = new ebmlMultiSlot(this);
    //     auto elem_sp = ebmlMultiSlot_sp(elem);
    //     elem->_validateArgs(kwargs);
    //     elem->_validateData(kwargs);
    ...
    // }
    //
    // ebmlMultiSlot_sp ebmlMultiSlotClass::operator()(const slotArg_l& args, const slotArg_d& kwargs) const {
    //     auto elem = new ebmlMultiSlot(this);
    //     auto elem_sp = ebmlMultiSlot_sp(elem);
    ...
    // }
    //
    // ebmlMultiSlot_sp ebmlMultiSlotClass::operator()(slotArg_l&& args, slotArg_d&& kwargs) const {
    //     auto elem = new ebmlMultiSlot(this);
    //     auto elem_sp = ebmlMultiSlot_sp(elem);
    //     elem->_validateArgs(args, kwargs);
    //     elem->_validateData(args);
    ...
    const ebmlElementClass* ebmlMultiSlotClass::clsBySlot(const std::string& key) const {
        if (this->_slots_by_name.count(key)) {
            ...
            auto& childClasses = slot->childClasses();

            if (childClasses.size()) {
                return childClasses.begin()->second;
            }
        }
    ...
    const ebmlElementClass* ebmlMultiSlotClass::clsBySlot(size_t index) const {
        if (index < this->_slotSpecs.size()) {
            auto& childClasses = this->_slotSpecs.at(index).childClasses();
    ...
    slotArg_t::~slotArg_t() {
        ...
        void _slot_t::_validate(const ebmlElement_sp& elem) const {
            if (!_spec->childClasses().isValid(elem)) {
    ...
    ebmlMultiSlot::ebmlMultiSlot(const ebmlMultiSlotClass* cls) : InstMixin<ebmlMultiSlotClass, ebmlMultiSlot, ebmlMasterElement>(cls) {
    ...
    slot_t::slot_t(const ebml::ptr<ebmlMultiSlot>& parent, _slot_t& slot) :_parent(parent), _slot(&slot) {}
    slot_t::slot_t(ebml::ptr<ebmlMultiSlot>&& parent, _slot_t& slot) :_parent(std::move(parent)), _slot(&slot) {}
    ...
    const_slot_t::const_slot_t(const ebml::ptr<const ebmlMultiSlot>& parent, const _slot_t& slot) :_parent(parent), _slot(&slot) {}
    const_slot_t::const_slot_t(ebml::ptr<const ebmlMultiSlot>&& parent, const _slot_t& slot) :_parent(std::move(parent)), _slot(&slot) {}
    ...
    const_slot_t::const_slot_t(slot_t&& other) :
        _parent(ebml_const_pointer_cast<const ebmlMultiSlot>(std::move(other._parent))), _slot(std::exchange(other._slot, nullptr)) {}
    ...
    ebmlElement& ebmlMultiSlot::_iterator::operator*() const {
        return **this->_iter;
    }
    ...
    template class ClsMixin<ebmlMultiSlotClass, ebmlMultiSlot, ebmlMasterElementClass>;
    template class InstMixin<ebmlMultiSlotClass, ebmlMultiSlot, ebmlMasterElement>;
    template class flexible_ptr<ebmlMultiSlot>;
    template class ebml_shared_ptr<ebmlMultiSlot>;
    template class ebml_weak_ptr<ebmlMultiSlot>;
}
#endif
