...
namespace ebml {
    ...
    // ebmlMapType<K, V, H, E, A, B>::ebmlMapType(const char* ebmlID, const std::wstring& name, const ebmlElementClass* paircls)
    // : ebmlMapType(unpackVint(ebmlID), name, paircls) {}

    template<typename K, typename V, typename H, typename E, typename A, typename B>
    ebmlMapType<K, V, H, E, A, B>::ebmlMapType(const char* ebmlID, const std::wstring& name, const ebmlPairType<const K, V>* paircls)
      : ebmlMapType(unpackVint(ebmlID), name, paircls) {}

    // template<typename K, typename V, typename H, typename E, typename A, typename B>
    // ebmlMapType<K, V, H, E, A, B>::ebmlMapType(ebmlID_t ebmlID, const std::wstring& name, const ebmlElementClass* paircls)
    // : ebmlTypeCRTP<ebmlMapType<K, V, H, E, A, B>, ebmlMap<K, V, H, E, A, B>, ebmlMasterElementClass>(ebmlID, name), _paircls(paircls->ptr<ebmlPairType<const K, V>>()) {
    //     this->_childTypes.add({this->_paircls, 0, -1});
    // }

    template<typename K, typename V, typename H, typename E, typename A, typename B>
    ebmlMapType<K, V, H, E, A, B>::ebmlMapType(ebmlID_t ebmlID, const std::wstring& name, const ebmlPairType<const K, V>* paircls)
      : ebmlTypeCRTP<ebmlMapType<K, V, H, E, A, B>, ebmlMap<K, V, H, E, A, B>, ebmlMasterElementType>(ebmlID, name), _paircls(paircls) {
        this->_childTypes.add({this->_paircls, 0, -1});
    }

    template<typename K, typename V, typename H, typename E, typename A, typename B>
    const ebmlPairType<const K, V>& ebmlMapType<K, V, H, E, A, B>::paircls() const {
        return *this->_paircls;
    }

    // template<typename K, typename V, typename H, typename E, typename A, typename B>
    // ebmlElement_sp ebmlMapType<K, V, H, E, A, B>::operator()() const {
    //     return ebmlElement_sp(this->_new());
    // }

    // template<typename K, typename V, typename H, typename E, typename A, typename B>
    // ebmlElement* ebmlMapType<K, V, H, E, A, B>::_new() const {
    //     return new ebmlMap<K, V, H, E, A, B>(this);
    // }

    template<typename K, typename V, typename H, typename E, typename A, typename B>
    ebmlMap<K, V, H, E, A, B>::ebmlMap(const ebmlMapType<K, V, H, E, A, B>* cls)
      : ebmlElementCRTP<ebmlMapType<K, V, H, E, A, B>, ebmlMap<K, V, H, E, A, B>, ebmlMasterElement>(cls) {}

    template<typename K, typename V, typename H, typename E, typename A, typename B>
    ebmlMap<K, V, H, E, A, B>::ebmlMap(const ebmlMap<K, V, H, E, A, B>& orig)
      : ebmlMap(&orig.cls()) {
    ...
    // template<typename K, typename V, typename H, typename E, typename A, typename B>
    // const ebmlMapType<K, V, H, E, A, B>* ebmlMap<K, V, H, E, A, B>::cls() const {
    //     return this->_cls->template ptr<ebmlMapType<K, V, H, E, A, B>>();
    // }
...
//
//     template<typename T>
//     ebmlPairType<T>::ebmlPairType(const char* ebmlID, const std::wstring& name, const ebmlElementClass* keycls, const ebmlElementClass* valcls)
//     : ebmlPairType(unpackVint(ebmlID), name, keycls, valcls) {}
//
//     template<typename T>
//     ebmlPairType<T>::ebmlPairType(const char* ebmlID, const std::wstring& name, const ebmlDataElementClass<T>* keycls, const ebmlElementClass* valcls)
//     : ebmlPairType(unpackVint(ebmlID), name, keycls, valcls) {}
//
//     template<typename T>
//     ebmlPairType<T>::ebmlPairType(ebmlID_t ebmlID, const std::wstring& name, const ebmlElementClass* keycls, const ebmlElementClass* valcls)
...
//
//     template<typename T>
//     ebmlPairType<T>::ebmlPairType(ebmlID_t ebmlID, const std::wstring& name, const ebmlDataElementClass<T>* keycls, const ebmlElementClass* valcls)
...
//
//     template<typename T>
//     ebmlElement_sp ebmlPairType<T>::operator()(const T& key, const ebmlElement_sp& value) const {
...
//
//     template<typename T>
//     ebmlElement_sp ebmlPairType<T>::operator()(const T& key, ebmlElement_sp&& value) const {
//         auto elem = new ebmlPair<T>(this);
//         auto elem_sp = ebmlElement_sp(elem);
//         elem->_validate(value);
...
//
//     template<typename T>
//     ebmlElement_sp ebmlPairType<T>::operator()(T&& key, ebmlElement_sp&& value) const {
//         auto elem = new ebmlPair<T>(this);
//         auto elem_sp = ebmlElement_sp(elem);
//         elem->_validate(value);
...
//
//     template<typename T>
//     ebmlElement* ebmlPairType<T>::_new() const {
//         return new ebmlPair<T>(this);
//     }
//
//     template<typename T>
//     const ebmlDataElementClass<T>* ebmlPairType<T>::keycls() const {
//         return this->_keycls->template ptr<ebmlDataElementClass<T>>();
//     }
//
//     template<typename T>
//     const ebmlElementClass* ebmlPairType<T>::valcls() const {
//         return this->_valcls;
//     }
//
//     template<typename T>
//     ebmlPair<T>::ebmlPair(const ebmlPairType<T>* cls) : ebmlMasterElement(cls) {}
//
//     template<typename T>
...
//
//     template<typename T>
//     const ebmlPairType<T>* ebmlPair<T>::cls() const {
//         return this->_cls->ptr<ebmlPairType<T>>();
//     }
//
//     // Element access
//     template<typename T>
...
//     ebmlMapType<K, void, H, E, A>::ebmlMapType(const char* ebmlID, const std::wstring& name, const ebmlElementClass* paircls)
//     : ebmlMapType(unpackVint(ebmlID), name, paircls) {}
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlMapType<K, void, H, E, A>::ebmlMapType(const char* ebmlID, const std::wstring& name, const ebmlPairType<const K>* paircls)
//     : ebmlMapType(unpackVint(ebmlID), name, paircls) {}
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlMapType<K, void, H, E, A>::ebmlMapType(ebmlID_t ebmlID, const std::wstring& name, const ebmlElementClass* paircls)
//     : ebmlMasterElementClass(ebmlID, name), _paircls(paircls->ptr<ebmlPairType<const K>>()) {
//         this->_childTypes.add({this->_paircls, 0, -1});
//     }
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlMapType<K, void, H, E, A>::ebmlMapType(ebmlID_t ebmlID, const std::wstring& name, const ebmlPairType<const K>* paircls)
//     : ebmlMasterElementClass(ebmlID, name), _paircls(paircls) {
//         this->_childTypes.add({this->_paircls, 0, -1});
//     }
//
//     template<typename K, typename H, typename E, typename A>
//     const ebmlPairType<const K>* ebmlMapType<K, void, H, E, A>::paircls() const {
//         return this->_paircls;
//     }
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlElement_sp ebmlMapType<K, void, H, E, A>::operator()() const {
//         return ebmlElement_sp(this->_new());
//     }
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlElement_sp ebmlMapType<K, void, H, E, A>::operator()(std::initializer_list<std::pair<K, ebmlElement_sp>> items) const {
...
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlElement_sp ebmlMapType<K, void, H, E, A>::operator()(const std::list<std::pair<K, ebmlElement_sp>>& items) const {
//         auto elem = new ebmlMap<K, void, H, E, A>(this);
//         auto elem_sp = ebmlElement_sp(elem);
...
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlElement_sp ebmlMapType<K, void, H, E, A>::operator()(std::list<std::pair<K, ebmlElement_sp>>&& items) const {
//         auto elem = new ebmlMap<K, void, H, E, A>(this);
//         auto elem_sp = ebmlElement_sp(elem);
...
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlElement_sp ebmlMapType<K, void, H, E, A>::operator()(const std::unordered_map<K, ebmlElement_sp, H, E, A>& items) const {
//         auto elem = new ebmlMap<K, void, H, E, A>(this);
//         auto elem_sp = ebmlElement_sp(elem);
...
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlElement_sp ebmlMapType<K, void, H, E, A>::operator()(std::unordered_map<K, ebmlElement_sp, H, E, A>&& items) const {
//         auto elem = new ebmlMap<K, void, H, E, A>(this);
//         auto elem_sp = ebmlElement_sp(elem);
...
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlElement* ebmlMapType<K, void, H, E, A>::_new() const {
//         return new ebmlMap<K, void, H, E, A>(this);
//     }
//
//     template<typename K, typename H, typename E, typename A>
//     ebmlMap<K, void, H, E, A>::ebmlMap(const ebmlMapType<K, void, H, E, A>* cls) : ebmlMasterElement(cls) {}
...
//
//     template<typename K, typename H, typename E, typename A>
//     const ebmlMapType<K, void, H, E, A>* ebmlMap<K, void, H, E, A>::cls() const {
//         return this->_cls->ptr<ebmlMapType<K, void, H, E, A>>();
//     }
//
//     template<typename K, typename H, typename E, typename A>
...
//     }
    ...
    template<typename K, typename V, typename H, typename E, typename A, typename B>
    ebmlMap<K, V, H, E, A, B>::ebmlMap(const ebmlMapType<K, V, H, E, A, B>* cls, const std::vector<std::pair<K, pair_value_t<V>>>& items)
      : ebmlElementCRTP<ebmlMapType<K, V, H, E, A, B>, ebmlMap<K, V, H, E, A, B>, ebmlMasterElement>(cls) {
        this->setData(items);
    }

    template<typename K, typename V, typename H, typename E, typename A, typename B>
    ebmlMap<K, V, H, E, A, B>::ebmlMap(const ebmlMapType<K, V, H, E, A, B>* cls, std::vector<std::pair<K, pair_value_t<V>>>&& items)
      : ebmlElementCRTP<ebmlMapType<K, V, H, E, A, B>, ebmlMap<K, V, H, E, A, B>, ebmlMasterElement>(cls) {
        this->setData(std::move(items));
    }

    template<typename K, typename V, typename H, typename E, typename A, typename B>
    ebmlMap<K, V, H, E, A, B>::ebmlMap(const ebmlMapType<K, V, H, E, A, B>* cls, const ebmlMapArgs<K, V, H, E, B>& items)
      : ebmlElementCRTP<ebmlMapType<K, V, H, E, A, B>, ebmlMap<K, V, H, E, A, B>, ebmlMasterElement>(cls) {
        this->setData(items);
    }

    template<typename K, typename V, typename H, typename E, typename A, typename B>
    ebmlMap<K, V, H, E, A, B>::ebmlMap(const ebmlMapType<K, V, H, E, A, B>* cls, ebmlMapArgs<K, V, H, E, B>&& items)
      : ebmlElementCRTP<ebmlMapType<K, V, H, E, A, B>, ebmlMap<K, V, H, E, A, B>, ebmlMasterElement>(cls) {
        this->setData(std::move(items));
    }
}
