...
namespace ebml {
    ...
    template<typename V>
    using pair_value_class_t = std::conditional_t<IS_EBML_TYPE(V), ebmlElementType, ebmlDataType<V>>;
     ...
     * @class ebmlPairType
     * @brief A template class representing a key-value pair.
     *
     * This template defines the structure for a key-value pair EBML element that is typically used as a child within an `ebmlMap`.
     * It is specialized with a key type `K` and a value type `V` to create a concrete pair element.
     */
    template<typename K, typename V>
    class ebmlPairType : public ebmlTypeCRTP<ebmlPairType<K, V>, ebmlPair<K, V>, ebmlMasterElementType> {
    public:
    ...
    private:
        const ebmlDataType<K>* _keycls;
        const pair_value_class_t<V>* _valcls;
        friend class ebmlPair<K, V>;
    };

    template<typename K, typename V>
    class ebmlPair : public ebmlElementCRTP<ebmlPairType<K, V>, ebmlPair<K, V>, ebmlMasterElement> {
    ...
    protected:
        ...
        // ebml::ptr<ebmlDataElementType<K>> _key;
        // ebml::ptr<pair_value_inst_t<V>> _value;
    ...
    public:
        ...
        // const ebmlPairType<K, V>* cls() const override;
    ...
    extern template class ebmlPairType<std::string, unsigned long long>;
    extern template class ebmlPair<std::string, unsigned long long>;
    // extern template class ebmlPairType<std::string, ebmlList>;
