...
namespace ebml {
    ...
    template<typename T>
    T& ebmlStruct::data(size_t offset) {
        ...
        if (elem == nullptr) {
            auto clss = slot.slotSpec().childTypes();
            ...
            auto dcls = dynamic_cast<const ebmlDataType<T>*>(cls);
    ...
    template<typename T>
    T& ebmlStruct::data(const std::string& key) {
    ...
    template<typename T>
    const T& ebmlStruct::data(size_t offset) const {
        if (offset >= this->_slots.size()) {
            std::string errmsg = "slot index '";
            errmsg += std::to_string(offset);
            errmsg += "' out of range";
    ...
    template<typename T>
    const T& ebmlStruct::data(const std::string& key) const {
        if (this->_slots_by_name.count(key) == 0) {
            std::string errmsg = "key '";
            errmsg += key;
            errmsg += "' not found";
