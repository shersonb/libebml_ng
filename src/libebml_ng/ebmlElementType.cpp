...
namespace ebml {
    ...
    seekMapBase* ebmlElementType::initSeekMap() const {
        return _seekHelper->initSeekMap();
    }

    seekData_t* ebmlElementType::makeSeekData(const parseString& parsed) const {
        return _seekHelper->makeSeekData(parsed);
    }

    seekData_t* ebmlElementType::makeSeekData(const parseString& parsed, const ebmlMasterElement_wp& parent) const {
        auto data = makeSeekData(parsed);
        data->parent = parent;
        return data;
    }

    seekData_t* ebmlElementType::makeSeekData(const parseString& parsed, ebmlMasterElement_wp&& parent) const {
        auto data = makeSeekData(parsed);
        data->parent = std::move(parent);
        return data;
    }

    seekData_t* ebmlElementType::makeSeekData(const parseFile& parsed) const {
        return _seekHelper->makeSeekData(parsed);
    }

    seekData_t* ebmlElementType::makeSeekData(const parseFile& parsed, const ebmlMasterElement_wp& parent) const {
        auto data = makeSeekData(parsed);
        data->parent = parent;
        return data;
    }

    seekData_t* ebmlElementType::makeSeekData(const parseFile& parsed, ebmlMasterElement_wp&& parent) const {
        auto data = makeSeekData(parsed);
        data->parent = std::move(parent);
        return data;
    }

    std::wstring ebmlElementType::repr() const {
    ...
    std::wstring ebmlElementType::minirepr() const {
        return std::wstring();
    }
}
