...
namespace ebml {
    ...
    // ebmlPairClass<K, V>::ebmlPairClass(const char* ebmlID, const std::wstring& name, const ebmlElementClass* keycls, const ebmlElementClass* valcls)
    // : ebmlPairClass(unpackVint(ebmlID), name, keycls, valcls) {}

    template<typename K, typename V>
    ebmlPairClass<K, V>::ebmlPairClass(const char* ebmlID, const std::wstring& name, const pair_key_class_t<K>* keycls, const pair_value_class_t<V>* valcls)
    : ebmlPairClass(unpackVint(ebmlID), name, keycls, valcls) {}

    // template<typename K, typename V>
    // ebmlPairClass<K, V>::ebmlPairClass(ebmlID_t ebmlID, const std::wstring& name, const ebmlElementClass* keycls, const ebmlElementClass* valcls)
    // : ClsMixin<ebmlPairClass<K, V>, ebmlPair<K, V>, ebmlMasterElementClass>(ebmlID, name), _keycls(keycls->ptr<pair_key_class_t<K>>()), _valcls(valcls->ptr<pair_value_class_t<V>>()) {
    //     this->_childClasses = {{keycls, 1, 1}, {valcls, 1, 1}};
    // }

    template<typename K, typename V>
    ebmlPairClass<K, V>::ebmlPairClass(ebmlID_t ebmlID, const std::wstring& name, const pair_key_class_t<K>* keycls, const pair_value_class_t<V>* valcls)
    : ClsMixin<ebmlPairClass<K, V>, ebmlPair<K, V>, ebmlMasterElementClass>(ebmlID, name), _keycls(keycls), _valcls(valcls) {
        this->_childClasses = {{keycls, 1, 1}, {valcls, 1, 1}};
    }

    // template<typename K, typename V>
    // ebmlElement_sp ebmlPairClass<K, V>::operator()(const K& key, const pair_value_t<V>& value) const {
    ...
    // template<typename K, typename V>
    // ebmlElement_sp ebmlPairClass<K, V>::operator()(K&& key, pair_value_t<V>&& value) const {
    //     auto elem = new ebmlPair<K, V>(this);
    //     auto elem_sp = ebmlElement_sp(elem);
    ...
    // template<typename K, typename V>
    // ebmlElement* ebmlPairClass<K, V>::_new() const {
    //     return new ebmlPair<K, V>(this);
    // }

    template<typename K, typename V>
    const pair_key_class_t<K>& ebmlPairClass<K, V>::keycls() const {
        return *this->_keycls;
    }

    template<typename K, typename V>
    const pair_value_class_t<V>& ebmlPairClass<K, V>::valcls() const {
        return *this->_valcls;
    }

    template<typename K, typename V>
    ebmlPair<K, V>::ebmlPair(const ebmlPairClass<K, V>* cls) : InstMixin<ebmlPairClass<K, V>, ebmlPair<K, V>, ebmlMasterElement>(cls) {}

    template<typename K, typename V>
    ebmlPair<K, V>::ebmlPair(const ebmlPairClass<K, V>* cls, const K& key, const pair_value_t<V>& value) : ebmlPair(cls) {
        _setData(key, value);
    }

    template<typename K, typename V>
    ebmlPair<K, V>::ebmlPair(const ebmlPairClass<K, V>* cls, const K& key, pair_value_t<V>&& value) : ebmlPair(cls) {
        _setData(key, std::move(value));
    }

    template<typename K, typename V>
    ebmlPair<K, V>::ebmlPair(const ebmlPairClass<K, V>* cls, K&& key, const pair_value_t<V>& value) : ebmlPair(cls) {
        _setData(std::move(key), value);
    }

    template<typename K, typename V>
    ebmlPair<K, V>::ebmlPair(const ebmlPairClass<K, V>* cls, K&& key, pair_value_t<V>&& value) : ebmlPair(cls) {
